...
export default function CalendarPage() {
  const { state, setState, loadProjects, loadPromotions } = useApp();
  const { getVisibleProjects, getCalendarPromotions } = useSelectors();
  ...
  // Load data on mount
  useEffect(() => {
    Promise.all([loadProjects(), loadPromotions()]).catch(console.error);
  }, []);

  // Rebuild calendar whenever promotions or filter changes
  useEffect(() => {
